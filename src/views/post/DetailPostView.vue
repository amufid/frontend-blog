  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="card flex">
      <div class="card flex justify-center">
        <div class="flex gap-2 items-center">
          ...
          <div>
            <i class="pi pi-chevron-right" style="font-size: 1rem; color: darkgray"></i>
          </div>
          <RouterLink to="/posts">Posts</RouterLink>
          <div>
            <i class="pi pi-chevron-right" style="font-size: 1rem; color: darkgray"></i>
          </div>
          <RouterLink to="" class="text-gray-400 cursor-default">Detail</RouterLink>
        </div>
      </div>
    </div>
    ...
    <div v-else class="card flex flex-col gap-4">
      <div class="w-auto flex gap-4">
        <Button
          severity="info"
          @click="goToUrl({ name: 'post-edit', slug: data.title, id: data.id })"
          >Edit</Button
        >
        <DeletePostModal :post="data.id" />
      </div>
...
const post_id = route.params.slug
